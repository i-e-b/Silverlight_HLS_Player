...
namespace HCS.StreamSource {
	...
	public class PlaylistMgr: IDisposable {
		...
		protected int SeekChunkIndex = 0; // starting index, set during 'Seek()'
		...
		public bool IsCompressedPlaylist {
			get {
				...
				else return PlaylistUrl.ToUpper().EndsWith(".GZ");
			}
		}
		...
		internal void SeekOffset (TimeSpan offset) {
			int diff = (int)(offset.TotalSeconds / ChunkDuration.TotalSeconds);
		...
		protected void StartTimer () {
			...
			timer.Interval = TimeSpan.FromTicks(ChunkDuration.Ticks * 2);
			timer.Tick += new EventHandler(ReadPlaylistAsync);
			timer.Start();
		}
		...
		private void ReadPlaylistAsync (object source, EventArgs e) {
			...
			System.Threading.ThreadPool.QueueUserWorkItem(new System.Threading.WaitCallback(this.ReadPlaylist));
		}
		...
		void wc_OpenReadCompleted (object sender, OpenReadCompletedEventArgs e) {
			...
			List<ChunkDetail> new_chunks = new List<ChunkDetail>();
		...
		private void ParseHeaderFromWebRequest (OpenReadCompletedEventArgs e, List<ChunkDetail> new_chunks) {
			...
			using (StreamReader b_sr = new StreamReader(e.Result)) {
				try {
					if (IsCompressedPlaylist) {
						InflaterInputStream x = new InflaterInputStream(b_sr.BaseStream);
					...
					string[] duration = sr.ReadLine().Split(new string[] { ":" }, StringSplitOptions.RemoveEmptyEntries);
					double seconds = 0.0;
					...
					this.ChunkDuration = TimeSpan.FromSeconds(seconds);
					...
					if (IsCompressedPlaylist) {
						InflaterInputStream x = sr.BaseStream as InflaterInputStream;
						if (x != null) {
							x.Close();
							x.Dispose();
						}
